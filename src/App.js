...
function App() {
  ...
  */
  const overwriteSubmission = (event) => {
    event.preventDefault();
    const conflictedIndex = eventList.findIndex(
      (e, i) =>
        e.driver === driver &&
        i !== taskIndex &&
  ...
  */
  const handleFormSubmission = (event) => {
    ...
    const isStartDateConflicted = eventList.some(
      (e, i) =>
        e.driver === driver &&
        i !== taskIndex &&
        moment(e.start).isBetween(
  ...
  const generateCSV = (scheduleDays) => {
    ...
    */
    ...
    dayCols.forEach((intervalDay) => {
      ...
      const pickupTaskCount = eventList.filter(
        (event) =>
          event.driver === selectedDriver &&
          event.taskType === "Pick-Up" &&
          moment(event["start"]).isBetween(
            intervalDay,
            moment(intervalDay).add(days, "days"),
            undefined,
            "[)"
          )
      ).length;

      const dropOffTaskCount = eventList.filter(
        (event) =>
          event.driver === selectedDriver &&
      ...
      const otherTaskCount = eventList.filter(
        (event) =>
          event.driver === selectedDriver &&
      ...
      const date = `Day ${moment(intervalDay).dayOfYear()} - Day ${moment(
        endIntervalDate
      ).dayOfYear()}`;
    ...
    <>
      <div className="header">
        ...
        <div className="header_options">
          ...
          <div className="driver_label2">
            <Button onClick={addNewEvent}>Add New Task</Button>
          </div>
      ...
      <div>
        ...
        >
          ...
          <Modal.Body>
            We have found a conflict. Please either overwrite the existing event
            or cancel the new one.
          </Modal.Body>
          <Modal.Footer>
            <Button variant="secondary" onClick={handleCloseErrorModal}>
              Go Back
            </Button>
            <Button variant="primary" onClick={overwriteSubmission}>
              Overwrite
            </Button>
          </Modal.Footer>
        </Modal>
      ...
      <div>
        <EditModalView
          ...
          handleCalendarStartDateTimeChange={handleCalendarStartDateTimeChange}
          taskEndDateTime={taskEndDateTime}
          handleCalendarEndDateTimeChange={handleCalendarEndDateTimeChange}
      ...
      <div>
        <CalendarView
          eventList={eventList.filter((task) => task.driver === selectedDriver)}
